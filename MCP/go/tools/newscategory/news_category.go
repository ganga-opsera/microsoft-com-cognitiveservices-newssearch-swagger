...
func News_categoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["headlineCount"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("headlineCount=%v", val))
		}
		...
		url := fmt.Sprintf("%s/news%s", cfg.BaseURL, queryString)
...
func CreateNews_categoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_news",
		...
		mcp.WithNumber("headlineCount", mcp.Description("The number of headline articles to return in the response. The default is 12. Specify this parameter only if you do not specify the [category](https://docs.microsoft.com/en-us/rest/api/cognitiveservices/bing-news-api-v7-reference#category) parameter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    News_categoryHandler(cfg),
	}
}
